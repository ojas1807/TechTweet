...
const ProjectPitch = () => {
    ...
    const [formFeedback, setFormFeedback] = useState(''); // Feedback for form submission
    ...
    const handleSubmit = (e) => {
        ...
        if (newProject.media && !isValidMediaUrl(newProject.media)) {
            setFormFeedback("Please enter a valid media URL (image or video).");
            return;
        }
        setProjects([...projects, newProject]);
        setNewProject({ title: '', description: '', media: '' }); // Reset form
        setFormFeedback("Project submitted successfully!"); // Success feedback
    };
    ...
    return (
        <div className="flex flex-col min-h-screen w-full bg-gray-100">
            ...
            <div className="flex flex-1 w-full container mx-auto px-4 py-6">
                {/* Sidebar */}
                ...
                <main className="flex-1 p-6 bg-white shadow-lg rounded-lg">
                    ...
                    <form onSubmit={handleSubmit} className="bg-gray-50 p-6 shadow-md rounded-lg">
                        ...
                        {/* Feedback message */}
                        {formFeedback && <p className="mt-4 text-green-500 font-semibold">{formFeedback}</p>}
                    </form>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                        {projects.filter(project => project.title.toLowerCase().includes(searchTerm.toLowerCase())).map((project, index) => (
                            <div key={index} className="bg-white shadow-md p-4 rounded transition-transform transform hover:scale-105">
                                ...
                                {project.media && (
                                    <div className="mt-4">
                                        {isValidMediaUrl(project.media) ? (
                                            project.media.match(/\.(mp4|webm|ogg)$/) ? (
                                            ...
                                            <p className="text-red-500">Invalid media URL.</p>
                                        )}
                                    </div>
                                )}
                            </div>
